...
public class PrefixCreate extends Window {
	 ...
	 * editor.
	 */
	public PrefixCreate(boolean newPrefix) {
	...
	/**
	 * The method calls from {@link #NamespacePrefixes} and sets the corresponding
	 * values of Namespace Prefix to the dialog.
	 * 
	 * @param selectedPrefix
	 *            NamespacePrefix that locate in the row of Namespace Prefixes table in which has been
	 *            pressed the button Edit.
	 */
	...
	private VerticalLayout buildMainLayout(final boolean newUser) {
		...
		prefixes = App.getApp().getNamespacePrefixes().getAllPrefixes();
		...
		createUser.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(newUser){
					...
					prefix = App.getApp().getNamespacePrefixes().createPrefix(prefixName.getValue().trim(), prefixUri.getValue().trim());
				}
				else{
					prefix = selectPrefix;
				...
				App.getApp().getNamespacePrefixes().save(prefix);
