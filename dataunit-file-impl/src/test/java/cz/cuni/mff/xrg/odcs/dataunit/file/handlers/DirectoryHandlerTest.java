...
public class DirectoryHandlerTest {
    ...
    public void testAddAllAsLink() throws DataUnitException {
        DirectoryHandler source = new DirectoryHandlerImpl(new File(dirToUse, "source"));
        // file to link 
        final File toLink = new File(dirToUse, "toLink");
        toLink.mkdirs();
        // prepare source
        final DirectoryHandler sourceDir = source.addNewDirectory("directory");
        final FileHandler sourceFile = source.addNewFile("file");
        final DirectoryHandler sourceLink = source.addExistingDirectory(toLink, new OptionsAdd(true));
        // test link rooted path
        assertEquals("/toLink", sourceLink.getRootedPath());

        // target
        DirectoryHandler target = new DirectoryHandlerImpl(new File(dirToUse, "target"));
