 ...
 * <p>
 * When overriding platform in session customizer, we need to specify some dummy value for {@code eclipselink.target-database} property in persistence.xml.
 * Specifying a platform will disable EclipseLink's auto-detection, which would override our platform spoofed in session customizer.
 * <p>
 * Session customizer detects whether we are running in a test. If so H2 in-memory database platform is used instead of Virtuoso or MySQL.
 * <p>
 * The customizer is really needed only for Virtuoso, as EclipseLink does not support this platform.
 * 
