 ...
 * <p>
 * Virtuoso caches sequence values in memory outside transaction context. Upon serious crash (e.g. SEGFAULT, SIGKILL, ...), it is unable to recover sequence
 * values. This causes inconsistency in database, which results in failure upon inserting a row with primary key generated from corrupted sequence.
 * <p>
 * To work around this problem, this aspect catches SQL errors in relevant business processes. If caught, sequences are reset to correct values according to
 * current state of database.
 * <p>
 * This aspect needs to run around Spring's transactional aspect, so we can catch errors that show up when committing transaction. The declared precedence
 * accomplishes this.
 * 
 * @author Jan Vojt
 */
...
public class VirtuosoSequenceSanitizerAspect {
     ...
     * AspectJ load-time or compile-time weaving, we could annotate {@link #updateSequences()} directly with {@link Transactional}.
     */
    @Autowired
     ...
     * 
     * @param pjp
     *            join point to proceed with
     * @param ex
     *            exception thrown by underlying database / JPA provider
