...
/**
 * We're storing multiple harvested data sets in one SOLR index (see http://wiki.apache.org/solr/MultipleIndexes for basic rationale why).
 * 
 * To distinguish between data sets (and to allow searching only in specific data set), field "type" is used.
 * 
 * Theory of operation:
 * <ol>
 * <li>data are harvested, parsed, ... and stored in a relevant bean {@code sk.opendata.odn.model.<Some>Record}</li>
 * <li>{@code SolrItem} is created from the "record bean" using appropriate {@code SolrItem} constructor</li>
 * <li>"SOLR items" are collected in list and pushed to SOLR</li>
 * </ol>
 * 
 * TODO: Usage of {@code SolrItemType} and {@code Currency} is causing exceptions like:
 * 
 * Caused by: java.lang.RuntimeException: Exception while setting value : ORGANIZATION_RECORD on private sk.opendata.odn.repository.solr.SolrItemType
 * sk.opendata.odn.repository.solr.SolrItem.type at org.apache.solr.client.solrj.beans.DocumentObjectBinder$DocField.set(DocumentObjectBinder.java:380) at
 * org.apache.solr.client.solrj.beans.DocumentObjectBinder$DocField.inject(DocumentObjectBinder.java:362) at
 * org.apache.solr.client.solrj.beans.DocumentObjectBinder.getBean(DocumentObjectBinder.java:67) at
 * org.apache.solr.client.solrj.beans.DocumentObjectBinder.getBeans(DocumentObjectBinder.java:47) at
 * org.apache.solr.client.solrj.response.QueryResponse.getBeans(QueryResponse.java:452) at sk.opendata.odn.ui.panel.ResultPanel.doSearch(ResultPanel.java:147)
 * 
...
public class SolrItem implements Comparable<SolrItem> {
    ...
    @Field("donor_psc")
    private String donorPsc; // TODO: we use "string" in SOLR schema => make sure we use same form by filtering out spaces (i.e. to prevent cases like "058 01"
                             // and "05801" being considered different PSC)
    @Field("donor_city")
     ...
     */
    public static SolrItem createSolrItem(AbstractRecord source) throws Exception {
